...
)
{
	...
	if (on) {
		...
		if (gpio_wl_reg_on >= 0) {
			/* set WL_REGON to output & LOW for reset */
			...
			/* delay 200ms & set WL_REGON to HIGH */
			mdelay(200);
...
static int ___dhd_wlan_set_carddetect(int present)
{
	int err = 0;
...
int dhd_wlan_init_gpio(void)
{
	...
	*/
	...
	gpio_wl_reg_on = of_get_named_gpio(root_node,"wlan_reg_on", 0);
...
#ifdef CUSTOMER_OOB
#ifdef CONFIG_BCMDHD_ANCHOR
	gpio_wl_host_wake = of_get_named_gpio(root_node, "wlan_host_wake", 0);
	if (!gpio_is_valid(gpio_wl_host_wake)) {
		printk("Invalied gpio pin : %d\n", gpio_wl_host_wake);
